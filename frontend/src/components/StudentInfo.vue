...
const nameRules = [(n) => !!n || 'Name is required'];
...
const pastCourseRules = [(p) => p.length > 0 || 'Select at least one past course'];

const availableCourseRules = [(c) => !!c || 'Desired course is required'];
...
const getPastCourses = async () => {
    try {
        const result = await fetch(`${import.meta.env.VITE_API_URL}courses/past`, {
            method: 'GET',
            headers: {
...
const getAvailableCourse = async () => {
    try {
        const result = await fetch(`${import.meta.env.VITE_API_URL}courses/available`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            },
...
const submitStudentSurvey = async () => {
    try {
        const reqBody = {
            ...
            past_courses: past_courses.value.map((courseId) => ({
                past_course_id: courseId,
            })),
        };
        ...
        if (data.status === 'ok') {
            ...
            alert(
                'It appears you have already completed this exam. If you feel this is in error, please contact us.'
            );
            return false;
        } else {
            alert('An error has occurred. Please contact us if this error persists.');
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <h3 class="text-subtitle-1 font-weight-bold mb-2">
                                        Past courses (select all that apply):
                                    </h3>
                                </v-col>
                            </v-row>
